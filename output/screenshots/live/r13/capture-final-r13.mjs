...
try {
  ...
  // 新建实验表单（含相似实验提示）
  {
    ...
    await page.screenshot({ path: path.join(__dirname, 'r13-modal-experiment-new.png'), fullPage: false });
    results.push(path.join(__dirname, 'r13-modal-experiment-new.png'));
    await ctx.close();
  }
} finally { await browser.close(); }
console.log('FINAL SHOTS:');
