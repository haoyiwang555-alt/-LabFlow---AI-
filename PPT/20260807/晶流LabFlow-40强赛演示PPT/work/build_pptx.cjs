...
const FONT = "微软雅黑";
const FONT_SIZE = { h1: 32, h2: 20, h3: 18, body: 16, small: 14, label: 12 };
const RADIUS = 0.17;

// ═══ 2. 配图（运行时从 img_map.json 读取） ═══
const IMG_MAP_PATH = path.join(__dirname, "img_map.json");
const IMG_MAP = fs.existsSync(IMG_MAP_PATH) ? JSON.parse(fs.readFileSync(IMG_MAP_PATH, "utf-8")) : {};

// imgOpts：图框由版面布局先行定死 → 校验文件存在 → 按图框宽高比从候选选比例最接近的一张
// → 自行计算 cover 居中裁剪，经 sizing:crop 表达
...
// 背景图片
const coverImg = imgOpts("cover_main", 0, 0, SLIDE_W, SLIDE_H);
if (coverImg) slide01.addImage(coverImg);
...
solutionSteps.forEach((step, i) => {
  const yPos = 1.8 + i * 1.35;
  addBullet(slide04, step, 1, yPos, 5.5);
});

// 右侧配图（宽高比1.19匹配原图）
const p4Img = imgOpts("p4_problem", 7, 1.8, 4.5, 4.8);
if (p4Img) slide04.addImage(p4Img);
...
// 左侧配图（宽高比1.19匹配原图）
const p5Img = imgOpts("p5_engine", 1, 1.8, 4.5, 4.8);
if (p5Img) slide05.addImage(p5Img);
...
engineFeatures.forEach((feature, i) => {
  const yPos = 1.8 + i * 1.35;
  addBullet(slide05, feature, 6.5, yPos, 5.5);
});
...
// 右侧配图（宽高比1.19匹配原图）
const p6Img = imgOpts("p6_graph", 7, 1.8, 4.5, 4.8);
if (p6Img) slide06.addImage(p6Img);
